void addCell(bool statusCell, int floor) {
  ...
  if (isWallCB(centralVLX) || forwRave) maze[maze.size() - 1].push_back(Trio(xRobot + dir[3].first, yRobot + dir[3].second, floor));
  ...
  if (isWallCB(backwardVLX) || backRave) maze[maze.size() - 1].push_back(Trio(xRobot + dir[1].first, yRobot + dir[1].second, floor));

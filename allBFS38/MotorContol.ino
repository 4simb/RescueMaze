...
void VLXandIMU(short int vv, float yy) {
  ...
  if (abs(rightVLX - leftVLX) < 40 && leftVLX < 145 && rightVLX < 145) {
    uVLX = -4.3 * (float)(rightVLX - leftVLX);
    //Serial.print("    ");
    //Serial.print("VLX");
  }
  else if (rightVLX > leftVLX && leftVLX < 145) uVLX = -3 * (float)(115 - leftVLX);
  else if (leftVLX > rightVLX && rightVLX < 145) uVLX = 3 * (float)(115 - rightVLX);
  else uVLX = 0;

  uVLX *= 0.92;

  if (uVLX == 0) u = 0.5 * uIMU;
  //else u = uVLX;
  else u = uVLX * 0.6 + uIMU * 0.4; //0.58 0.42
  leftMotorValue = vv - u;
  rightMotorValue = vv + u;
}

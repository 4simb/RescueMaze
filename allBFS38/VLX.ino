...
void getVLX() {
  ...
  if (leftVLX > 160) tMVLeftOld = millis();
  if (rightVLX > 160) tMVRightOld = millis();
  
  if (millis() - tMVLeftOld >= 1000) lNormal = true;
  else lNormal = false;
  if (millis() - tMVRightOld >= 1000) rNormal = true;
  else rNormal = false;
}

int valueForward(int centr) {
  for (offerI = offerSize - 2; offerI >= 0; --offerI) {
    if (centr > offerForward[offerI] && centr - offerForward[offerI] > 105) { //100
...
int valueBackward(int centr) {
  for (offerI = 0; offerI < offerSize - 1; ++offerI) {
    if (centr < offerBackward[offerI] && offerBackward[offerI] - centr > 105) { //100
...
bool vlxOKBack(int bvlx, int bvlxOld, int enc, int fvlx) {
  return (bvlx < valueBackward(bvlxOld) || (float)enc < angleEnc * 0.7) && fvlx >= 65;
}

bool vlxOKForw(int fvlx, int fvlxOld, int enc) {
  return (fvlx > valueForward(fvlxOld) || (float) enc < (float)angleEnc * 0.7) && fvlx >= 65;
}
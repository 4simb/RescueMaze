...
void VLXandIMU(short int vv, float yy) {
  ...
  else if (rightVLX > leftVLX && leftVLX < 165) uVLX = -3.2 * (float)(110 - leftVLX);
  else if (leftVLX > rightVLX && rightVLX < 165) uVLX = 3.2 * (float)(110 - rightVLX);
  else uVLX = 0;
  //uVLX *= -1;

  if (uVLX == 0) u = uIMU;
  //else u = uVLX;
  else u = uVLX * 0.55 + uIMU * 0.45; //0.58 0.42
  leftMotorValue = vv - u;
  rightMotorValue = vv + u;
}
...
void Turns() { //85
  ...
  u = err * 2.9;
  if (abs(u) < 65) u = 65 * signYaw(zeroYaw, yaw);
  leftMotorValue = -u;
  rightMotorValue = u;
}

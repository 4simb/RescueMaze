...
class MPU9250_ {
	...
	float realPitch, realYaw, realRoll;
    ...
    double beta = 0.25f;  // 0.16f 0.25f 0.24f 0.23f
...
public:
	...
	void calibrateGyro(uint64_t duration) {
		...
		_gx_offset = (float)_gx / duration;
		_gy_offset = (float)_gy / duration;
		_gz_offset = (float)_gz / duration;
		
		/*EEPROM.put(0, _gx_offset);
		EEPROM.put(1, _gy_offset);
		EEPROM.put(2, _gz_offset);*/
	}
    ...
    bool update() {
		...
		*/
		...
		madgwick(deltaT, gn, ge, gd, an, ae, ad, q);
		
        return true;
    }

	float getXOffset() { return _gx_offset; }
	float getYOffset() { return _gy_offset; }
	float getZOffset() { return _gz_offset; }
	
	float setXOffset(float xx) { _gx_offset = xx; }
	float setYOffset(float yy) { _gy_offset = yy; }
	float setZOffset(float zz) { _gz_offset = zz; }
	...
	void madgwick(float dT, float xg, float yg, float zg, float xa, float ya, float za, float* q)
	{
...
public:
    void update_accel_gyro() {
        ...
        a[0] = (float)raw_acc_gyro_data[0] * acc_resolution + 0.02;// - 0.04;  // get actual g value, this depends on scale being set
        a[1] = (float)raw_acc_gyro_data[1] * acc_resolution + 0.01;// + 0.03;
        a[2] = (float)raw_acc_gyro_data[2] * acc_resolution + 0.10;// + 0.11;

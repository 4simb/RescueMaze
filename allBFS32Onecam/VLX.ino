...
void getVLX() {
  ...
  if (leftVLX > 170) tMVLeftOld = millis();
  if (rightVLX > 170) tMVRightOld = millis();
...
int valueForward(int centr) {
  for (offerI = offerSize - 2; offerI >= 0; --offerI) {
    if (centr > offerForward[offerI] && centr - offerForward[offerI] > 100) {
...
int valueBackward(int centr) {
  for (offerI = 0; offerI < offerSize - 1; ++offerI) {
    if (centr < offerBackward[offerI] && offerBackward[offerI] - centr > 100) {

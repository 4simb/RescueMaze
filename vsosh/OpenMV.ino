void readMVLeft() {
  if (Serial2.available()) {
    ...
    if (!goSlide && lNormal && state != 0 && state != 1 && state != 6 && state != 7 && !isMVImpact && (abs(pitch) > 165 || IGNORE_PITCH)) {
      Serial.print("left:");
...
void readMVRight() {
  if (Serial3.available()) {
    ...
    Serial << "RIGHT ";
    //Serial.print("right ");
    if (!goSlide && rNormal && state != 0 && state != 1 && state != 6 && state != 7 && !isMVImpact && (abs(pitch) > 165 || IGNORE_PITCH)) {
      Serial.print("right:");

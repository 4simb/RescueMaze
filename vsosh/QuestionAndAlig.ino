...
void alignment(float napr) {
  ...
  if (napr == FORWARD) {
    ...
    if ((!digitalRead(LBUTTON_UP) && !digitalRead(RBUTTON_UP)) || centralVLX < 55) secondAlignment = true;  //55
    ...
    if (backwardVLX < 37 || (backMicrPushed(MICROSWITCH_RIGHT) && backMicrPushed(MICROSWITCH_LEFT))) secondAlignment = true;
    //                27

...
threshold_black = (16, 0, -128, 127, -128, 127)#(26, 0, -128, 127, -128, 127)#(28, 0, -128, 127, 127, -128)
...
while(True):
    ...
    for yb in img.find_blobs([threshold_black], area_threshold = 300, merge = True, margin = 100):#area_threshold = 2000):
        if yb.pixels() > 20 and yb.w()/yb.h() > 1/2 and yb.w()/yb.h() < 3/2:
            ...
            #MIDDLE
            roi2 = (max(0, xList), max(0, yList + int(0.5 * yb.h()) - 2), min(img.width(), yb.w()), 18)
            count2 = 0
            for ybN in img.find_blobs([threshold_black], roi = roi2, area_threshold = 0, pixel_threshold = 0, x_stride = 1, merge = True, margin = 100):
                count2 += 1;
                #img.draw_rectangle(ybN.x(), ybN.y(), ybN.w(), ybN.h(), color = (255, 255, 255))

            #UP
            roi3 = (max(0, xList - 3), max(yList - 3, 0), min(img.width(), yb.w() + 6), 18)
            count3 = 0
            for ybN in img.find_blobs([threshold_black], roi = roi3, area_threshold = 0, pixel_threshold = 0, x_stride = 1, merge = True, margin = 100):
                count3 += 1;
                #img.draw_rectangle(ybN.x(), ybN.y(), ybN.w(), ybN.h(), color = (255, 255, 255))

...
def inThreshold(n):
    return bool(n >= threshold_black[0] and n <= threshold_black[1])
...
while(True):
    ...
    for elements in copies:
        ...
        up = (elements.x() + int(0.5 * elements.w()), min(elements.y() + 1, img.height()))
        middle = (elements.x() + int(0.5 * elements.w()), elements.y() + int(0.5 * elements.h()))
        down = (elements.x() + int(0.5 * elements.w()), max(elements.y() + elements.h() - 2, 0))
        ...
        if inThreshold(colorDown) and inThreshold(colorMiddle) and inThreshold(colorUp):
            print("S")
            img.draw_rectangle(elements.x(), elements.y(), elements.w(), elements.h())

        #colorMiddle = img.get_pixel(middle)
        if not(inThreshold(colorDown)) and inThreshold(colorMiddle) and not(inThreshold(colorUp)):
            print("H")
            img.draw_rectangle(elements.x(), elements.y(), elements.w(), elements.h())

        #colorDown = img.get_pixel(down)
        if inThreshold(colorDown) and not(inThreshold(colorMiddle)) and not(inThreshold(colorUp)):
            print("U")
            #print(elements.pixels())
            img.draw_rectangle(elements.x(), elements.y(), elements.w(), elements.h())

        #print(inThreshold(colorDown), )
        img.draw_circle((elements.x() + int(0.5 * elements.w()), elements.y() + int(0.5 * elements.h()), 2), color = 255, fill = True)
        img.draw_circle((elements.x() + int(0.5 * elements.w()), min(elements.y() + 1, img.height()), 2), color = 255, fill = True)
        img.draw_circle((elements.x() + int(0.5 * elements.w()), max(elements.y() + elements.h() - 2, 0), 2), color = 255, fill = True)
        #img.draw_circle(elements.x() + int(0.5 * elements.w()), min(elements.y() + 2, img.height()), radius = 5)#, color = 0, fill = True)
        #img.draw_circle(elements.x() + int(0.5 * elements.w()), elements.y() + int(0.5 * elements.h()), radius = 5)#, color = 127, fill = True)
        #img.draw_circle(elements.x() + int(0.5 * elements.w()), max(elements.y() + elements.h() - 5, 0), radius = 5)#, color = 255, fill = True)
        #if findU:
        #    print("U")

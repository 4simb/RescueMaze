...
threshold_black = (32, 0, -7, 7, -11, 8)
threshold_yellow = (0, 100, -128, 127, 127, 49)
threshold_green = (0, 100, -128, -22, 49, -128)
threshold_red = (0, 100, 15, 127, -128, 127)
...
while(True):
    ...
    for yb in img.find_blobs([threshold_black], area_threshold = 400):#area_threshold = 2000):
        if yb.pixels() > 10 and yb.pixels() < 1000:
        ...
        if count == 3:
            ...
            if count2 == 1 or count2 == 2:
                ...
                if (count2 == 1 and count3 == 2) or (count2 == 2 and count3 == 1):
                    #UP
                    roi4 = (max(0, xList[i] - 3), max(0, yList[i] - 1), min(img.width(), elements.width() + 5), 10)
                    count4 = 0
    ...
    if not(state):
        for ybb in img.find_blobs([threshold_yellow], merge = True, margin = 10, area_threshold = 700):
            if ybb.pixels() < 2000:
                img.draw_rectangle(ybb.x(), ybb.y(), ybb.w(), ybb.h(), color = (255, 0, 0))
                print("Y")
                uart.write("Y")
                state = True
                break

        for ybb in img.find_blobs([threshold_green], merge = True, margin = 10, area_threshold = 700):
            if ybb.pixels() < 2000:
                img.draw_rectangle(ybb.x(), ybb.y(), ybb.w(), ybb.h(), color = (255, 0, 0))
                print("G")
                uart.write("G")
        ...
        for ybb in img.find_blobs([threshold_red], merge = True, margin = 10, area_threshold = 700):
            if ybb.pixels() < 2000:
                img.draw_rectangle(ybb.x(), ybb.y(), ybb.w(), ybb.h(), color = (255, 0, 0))
                print("R")
                uart.write("R")

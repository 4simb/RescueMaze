...
void caseForward() {
  ...
  if (!isMVImpact && 
      (byBack && backwardVLX >= backwardOld + abs(valueBackward(backwardOld) - backwardOld) / 2)
  ...
  if (digitalRead(BUTTON_LEFT) && digitalRead(BUTTON_RIGHT)) fromForward = true;
                  ...
                  || (left - l0) / 2 + (right - r0) / 2 + rememberEnc < 1450)) && centralVLX >= 65) {
    ...
    VLXandIMU(velPitch, setting);
    Serial.print(" FORW");
  } else if (byEnc && ((left - l0) / 2 + (right - r0) / 2) + rememberEnc < 3050 && centralVLX >= 65) {
    VLXandIMU(velPitch, setting);
...
void someForw() {
  ...
  if ((left - l0) / 2 + (right - r0) / 2 > 710) { //850
    state = 4;
    timer = millis();
    someForwFirst = true;
    addCell(true, floors);

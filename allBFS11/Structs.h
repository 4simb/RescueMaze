...
class CameraParams {
  public:
    ...
    int fvlx, bvlx;
    bool camera;
    CameraParams(int first, int second, int floor, int dir, int fvlx, int bvlx, bool cam) {
      this->first = first;
      this->second = second;
      this->floor = floor;
      this->dir = dir;
    ...
    bool isEqual(int first, int second, int fl, int dir, int forw, int back, bool cam) {
      return abs(this->first - first) + abs(this->second - second) <= 1 
             && this->floor == fl && this->dir == dir && abs(fvlx - forw) < 250 && abs(bvlx - back) < 250 && this->camera == cam;
    }
};

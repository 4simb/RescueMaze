void addCell(bool statusCell) {
  ...
  smezh[smezh.size() - 1].push_back(pair(xRobot, yRobot));
  //LEFT BACK RIGHT FORW

  if (isWallLR(leftVLX)) smezh[smezh.size() - 1].push_back(pair(xRobot + dir[0].first, yRobot + dir[0].second));
  if (isWallCB(backwardVLX)) smezh[smezh.size() - 1].push_back(pair(xRobot + dir[1].first, yRobot + dir[1].second));
  if (isWallLR(rightVLX)) smezh[smezh.size() - 1].push_back(pair(xRobot + dir[2].first, yRobot + dir[2].second));
  if (isWallCB(centralVLX)) smezh[smezh.size() - 1].push_back(pair(xRobot + dir[3].first, yRobot + dir[3].second));

...
bool mvInUsedCells(bool mvNum) {
  ...
  /*char mvNormal, mvOpposite;
  if (!mvNum) {
    mvNormal = openMVReadRight;
    mvOpposite = openMVReadLeft;
  } else {
    mvNormal = openMVReadLeft;
    mvOpposite = openMVReadRight;
  }*/

  int x = xRobot + fEditor.first, y = yRobot + fEditor.second;
  for (int i = 0; i < wasMV.size(); ++i) {
    if (wasMV[i].isEqual(x, y, floors, robotDirection, mvNum, openMVReadLeft)
        || wasMV[i].isEqual(x, y, floors, robotDirection, mvNum, openMVReadRight)
        || wasMV[i].isEqual(x, y, floors, getOppositeDir(robotDirection), !mvNum, openMVReadLeft)
        || wasMV[i].isEqual(x, y, floors, getOppositeDir(robotDirection), !mvNum, openMVReadRight)) {
      return true;
    }

    /*Serial << wasMV[i].first << " " << wasMV[i].second << " " << wasMV[i].floor << " " << wasMV[i].dir << " "
           << " " << wasMV[i].camera << " " << wasMV[i].letter << endl;
    Serial << xRobot << " " << yRobot << " " << floors << " " << robotDirection << " "
           << " " << mvNum << " " << mvNormal << endl;
    Serial << xRobot << " " << yRobot << " " << floors << " " << getOppositeDir(robotDirection) << " "
           << " " << !mvNum << " " << mvOpposite << endl;
    Serial << endl;*/
  }
